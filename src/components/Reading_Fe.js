...
const Reading_Fe = () => {
  ...
  const calculateBandScore = (correctAnswers) => {
    // Official IELTS Reading Academic band score criteria
  ...
  const fetchExamHistory = async (examId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter for this specific exam, exclude forecast results, and get all attempts
        const examHistory = allHistory
          .filter(result => result.exam_id === examId && !result.is_forecast)
          .sort((a, b) => new Date(b.completion_date) - new Date(a.completion_date))
          .map((result, index, array) => ({
            ...result,
            band_score: calculateBandScore(result.total_score)
            // Use attempt_number from backend instead of calculating here
          }));

        setExamHistories(prev => ({ ...prev, [examId]: examHistory }));
      } else {
  ...
  const filteredTests = tests
    .filter(test => {
      ...
      if (test.partTitles) {
        return Object.values(test.partTitles).some(pt => pt && pt.toLowerCase().includes(query));
      }
      return false;
    })
    .sort((a, b) => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="flex flex-col items-center justify-center p-8 rounded-2xl bg-white shadow-lg">
          ...
          <div className="text-xl font-medium text-gray-700 mb-2">Loading Reading tests...</div>
                              ...
                              <>
                                {examHistories[test.id].slice(0, 2).map((result, idx) => (
                                  <button
                                    ...
                                    className="w-full p-3 text-left hover:bg-blue-50/50 border-b border-gray-50 transition-colors group"
                                  >
