...
const Listening_Fe = () => {
      ...
      if (!userId) {
        try {
          ...
          if (profileRes.ok) {
            ...
            // Re-persist so we don't repeat this round-trip
            if (userId) {
          ...
          // Token rejected or profile fetch failed — force re-login rather
          // than silently degrading to a permissive role.
          navigate('/login');
          return;
        } catch (e) {
          navigate('/login');
  ...
  const confirmRetake = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetchWithTimeout(`${API_BASE}/student/listening/exam/${testToRetake.id}/retake`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentTests.map((test, index) => (
            <div
              key={test.id}
              className={`bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-lime-200 transition-all duration-300 flex flex-col relative ${examHistoryDropdowns[test.id] ? 'z-50' : 'z-10'} ${isLimitedUser && (index + indexOfFirstTest) >= 6 ? 'opacity-80' : ''
                }`}
            >
              {/* Card Body */}
              <div className="p-5 flex-grow flex flex-col">
                ...
                <div className="flex flex-col gap-2 mb-2 mt-2">
                  {[1, 2, 3, 4].map((partNum) => {
                    ...
                    const title = stripHtmlTags(rawTitle) || 'Empty title';
